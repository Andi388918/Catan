...
namespace catan
{
    ...
    namespace copy
    {
        ...
        static void from_grid_graph(size_type rows, size_type columns, Graph& graph)
        {
    ...
    static void add_diagonal_edges(size_type rows, size_type columns, Graph& graph)
    {
        const std::vector<std::pair<size_type, size_type>> directions = {
            {1, 1}, {-1, -1}, {1, -1}, {-1, 1}
        };

        for (size_type row{}; row < rows; ++row)
        {
            for (size_type column{}; column < columns; ++column)
            {
                ...
                if (margin <= j && j < columns - margin)
                {
                    if (odd_row)
                    {
                        if (j % 2 != margin % 2)
                        {
                            ...
                            graph[vertex(i * columns + j, graph)].type = NodeType::hex;  /* define type of node */
                        ...
                        else
                        {
                            ...
                            size_type upper_vertex{ (i - 1) * columns + j };
                            size_type lower_vertex{ (i + 1) * columns + j };
                    ...
                    else
                    {
                        ...
                        graph[vertex(i * columns + j, graph)].type = NodeType::settlement;   /* define type of node */
    ...
    CatanGraph::CatanGraph(size_type radius) : radius{ radius }, rows{ 3 + 4 * radius }, columns{ rows }
    {
